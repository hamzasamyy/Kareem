"""System tray icon: a visible sign Jarvis is running, with a Quit option.
...
def start_tray_icon(launch_cmd: list[str] | None = None) -> bool:
    ...
    started with — "Restart Jarvis" reuses it so a silent (pythonw
    ...
    def restart_jarvis(icon, item):
        ...
        if launch_cmd:
            try:
                ...
                # for a few seconds (see jarvis/singleton.py), so it's fine
