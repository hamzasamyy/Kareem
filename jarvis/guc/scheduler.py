...
from jarvis import config, session_log, trackers
from jarvis.guc import auth, cms_scraper, portal_scraper, mail_scraper, finance_scraper, parser, sync
...
def run_check_cycle() -> dict:
    ...
    (same pattern as other Jarvis activity). Never raises — every failure
...
def start_background_loop() -> bool:
    ...
    _thread = threading.Thread(
        target=_loop, args=(_stop_event,), daemon=True, name="jarvis-guc-checker"
    )
    _thread.start()
    return True
