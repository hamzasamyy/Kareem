"""
Bridge letting a plain tool function (jarvis/tools/*.py, no access to the
WebSocket or any per-connection state) push a UI action to whichever browser
tab is currently running the active turn.

Mirrors jarvis.safety's set_ask_fn/_voice_ask_fn pattern exactly: the web
server installs a push function for the duration of each turn (see
jarvis/web/server.py's run_turn), and tools call the module-level function
