...
created it, but jarvis/web/server.py spawns a NEW worker thread per chat
...
(a visible Chromium window) by default so the user can see what Jarvis
    ...
    def _ensure_started(self):
        with self._lock:
            ...
            self._thread = threading.Thread(
                target=self._run, args=(ready,), daemon=True, name="jarvis-browser"
            )
            self._thread.start()
...
def browser_click(description: str) -> str:
    from jarvis.safety import classify_click_intent, guard, refuse_if_hard_blocked_click
...
def browser_type(description: str, text: str) -> str:
    from jarvis.safety import refuse_if_credential_like
