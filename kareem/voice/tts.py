"""
Text-to-speech. Uses the engine named by TTS_ENGINE in kareem/config.py
...
Kokoro's model and voices files belong in kareem/voice/models/kokoro/. Piper
voice files belong directly in kareem/voice/models/ so its model search can
...
class SentenceStreamer:
    """Plays sentences from a queue on a background thread, so Kareem can
    ...
    def wait(self, timeout: float = 120):
        ...
        the wake word early, so Kareem heard itself)."""
