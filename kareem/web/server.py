"""
Kareem web server — FastAPI + one WebSocket, wrapping the EXISTING engine.

This file does not contain any AI logic. It only:
  1. serves the static page (kareem/web/static/) at http://127.0.0.1:8000
...
(see kareem/guc/sync.py's section_for_item()), and Finance items are
fully read-only. The general (non-University) safety confirmation gate
in kareem/safety.py is untouched by this.
"""
...
# that isn't Kareem's own page closes that hole without needing a token.
...
def _transcribe_backend(audio_bytes: bytes) -> str:
    """Decode one browser MediaRecorder blob and use desktop Kareem's STT."""
    import numpy as np
...
def create_app(agent, agent_lock: threading.Lock) -> FastAPI:
    ...
    """
    from kareem import briefing, config, memory, safety, session_log, trackers
    ...
    async def guc_check():
        ...
        threading.Thread(
            target=guc_scheduler.run_check_cycle, daemon=True, name="kareem-guc-manual-check"
        ).start()
        return {"started": True}
    ...
    async def set_guc_item_seen(item_id: str, body: dict):
        ...
        # manual toggle (see kareem/guc/sync.py's sync_finances).
    ...
    async def websocket_endpoint(ws: WebSocket):
        ...
        if not _origin_is_allowed(ws.headers.get("origin")):
            ...
            # other tool), so a mismatched Origin (a page other than Kareem's
        ...
        if not session_log.current_session_materialized():
            ...
            if briefing_text and not session_log.current_session_materialized():
                ...
                briefing_text = None
        from kareem.voice.tts import get_active_tts_engine
...
if __name__ == "__main__":
    ...
    # app —  python -m kareem.web.server  (from the Kareem folder)
    import sys
