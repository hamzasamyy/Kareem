/* ============================================================================
   Kareem web UI — plain JavaScript, no build step.

   Talks to the backend over one WebSocket (protocol documented in
   kareem/web/server.py). Reconnects automatically if the server restarts.
...
function addMessage(who, text, time = null, images = []) {
  ...
  meta.innerHTML = `<span class="who">${who === "you" ? "YOU" : "KAREEM"}</span>` +
...
function renderConversation(events) {
  for (const event of events) {
    ...
    else if (event.type === "assistant_reply") addMessage("kareem", event.text || "", event.ts);
...
function endStreamingReply(finalText) {
  ...
  if (streamingMsg) {
    ...
    addMessage("kareem", finalText);   // reply that never streamed (e.g. Ollama)
  }
  gotTokens = false;
  setBusy(false);
  scheduleMicArm();
  ...
  ws.onmessage = (event) => {
    ...
    switch (msg.type) {
      ...
      case "error":
        ...
        addMessage("kareem", msg.message).classList.add("error");
  ...
  try {
    ...
    for (const event of (Array.isArray(data.events) ? data.events : [])) {
      if (event.type === "user_message" || event.type === "assistant_reply") {
        const msg = document.createElement("div");
        msg.className = `history-msg ${event.type === "user_message" ? "you" : "kareem"}`;
        const meta = document.createElement("div");
        meta.className = "msg-meta";
        meta.textContent = `${event.type === "user_message" ? "YOU" : "KAREEM"}  ${loggedTime(event.ts)}`;
        const body = document.createElement("div");
        body.className = "msg-body";
...
function gucItem(item, section) {
  ...
  // background re-scrape (kareem/guc/sync.py's sync_finances). There is
  ...
  if (!simple) {
    ...
    // purely status-driven, see kareem/guc/sync.py).
...
// Driven by the server's "navigate_app" WS message (kareem/tools/apps.py's
// navigate_app tool, via kareem/web/bridge.py) — lets the brain switch the
// web UI's own screens by name instead of only being able to describe them.
// Keep the section keys in sync with APP_SECTIONS in kareem/tools/apps.py.
...
let voiceFirstMode = localStorage.getItem("kareem-voice-first") !== "false";
...
function confirmationVoiceAvailable() {
  ...
  // no listener was ever started — for anyone using Kareem primarily by
...
function stopConfirmationVoice() {
  ...
  if (session) {
    if (session._kareemCancel) session._kareemCancel();
    session.onend = null;
  ...
  if (confirmationRecorder && confirmationRecorder.state !== "inactive") {
    confirmationRecorder._kareemSend = false;
...
async function startConfirmationBackendListen() {
  ...
  recorder._kareemSend = false;
  recorder._kareemConfirmation = true;
  ...
  recorder.onstop = async () => {
    const shouldSend = recorder._kareemSend;
    if (mediaRecorder === recorder) mediaRecorder = null;
    if (confirmationRecorder === recorder) confirmationRecorder = null;
    setListening(false);
...
function startConfirmationBrowserListen() {
  ...
  session._kareemCancel = () => { finished = true; clearTimers(); };
  const finalize = () => {
    if (finished) return;
    finished = true;
    clearTimers();
...
function setVoiceMode(enabled) {
  ...
  localStorage.setItem("kareem-voice-first", String(enabled));
...
function drawOrb(time) {
  ...
  if (mediaRecorder && mediaRecorder.state === "recording") {
    ...
    const confirmationRecording = mediaRecorder._kareemConfirmation;
...
function normalizeForSpeech(text) {
  // Mirrors kareem/voice/tts.py's normalize_for_speech(): the web HUD speaks
...
// Mirrors kareem/brain.py's SentenceBuffer._ABBREVIATIONS -- without this,
...
function stopListening() {
  if (recognition) {
    if (recognition._kareemCancel) recognition._kareemCancel();
...
function stopBackendRecording(sendAudio) {
  ...
  recorder._kareemSend = sendAudio;
