...
Every added feature registers more tools, and kareem.agent used to hand the
...
Keep CATEGORIES in sync with kareem/tools/*.py — a tool whose name isn't
...
CATEGORIES: dict[str, set[str]] = {
    ...
    # search was never offered and Kareem confidently answered from stale
    ...
    # it isn't keyword-gated at all anymore. See kareem/tools/web.py.
    #
    ...
    # than a silently-failed calendar action. See kareem/tools/calendar.py.
...
def select_tools_for_message(tool_specs: list[dict], text: str, brain: str) -> list[dict]:
    ...
    kareem.agent.SYSTEM_PROMPT_SMALL) — gets the keyword-filtered subset from
    filter_tool_specs().
    """
    if brain == "ollama":
