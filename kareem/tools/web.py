...
@register({
    ...
    "description": (
        ...
        "Use this for current events or anything you don't know. "
        "Answer from the results you already have whenever you can: read "
        "the titles/snippets FIRST — most factual questions (a score, a "
        "winner, a current status) are answered directly by the snippet, "
        "so use it rather than making more tool calls just to be extra "
        "sure. Only call fetch_page when snippets are genuinely "
        "insufficient, and even then fetch just ONE promising result. "
        "Never escalate to browser_open/browser_read for a plain page "
        "lookup — those open a real, visible browser window and are far "
        "slower; they're for clicking, typing, or a login, not for "
        "reading text fetch_page already covers. If fetch_page still "
        "isn't enough, say so rather than reaching for the browser tools. "
        ...
        "STOP and tell the user you couldn't find specific data for it "
        "(general/regional results you already have are still worth "
        "sharing) — don't keep trying rephrasings one at a time just "
        "because each attempt seemed locally reasonable; a hard-to-find "
        "topic usually has no better answer no matter how many ways you "
        "ask, and each extra attempt only delays a response you could "
        "give sooner."
    ),
    "parameters": {
        "type": "object",
