"""Single-instance enforcement for Kareem on Windows.
...
_DEFAULT_MUTEX_NAME = "Global\\KareemSingleInstanceMutex"
...
# release the mutex while Kareem is still running.
_mutex_handle = None


def acquire(retry_seconds: float = 3.0, poll_interval: float = 0.3,
            name: str = _DEFAULT_MUTEX_NAME) -> bool:
    """Try to become the one running Kareem instance.

    Returns True if this process now holds the lock, False if another
    Kareem process already holds it. Retries for up to `retry_seconds`
    (a few seconds by default) so "Restart Kareem" — which starts the new
    ...
    against duplicate background threads if you start Kareem twice, which is
    a nice-to-have, not something that should block Kareem from running at
    all on a platform this check doesn't support yet.
    """
    if sys.platform != "win32":
        print("(single-instance protection is Windows-only — skipping; "
              "avoid starting Kareem twice from separate terminals.)")
        return True
