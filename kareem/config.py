"""
Kareem configuration — this is the ONE file you edit to change how Kareem behaves.

Every setting has a comment explaining what it does. Change a value, save the
file, restart Kareem (`python main.py`), and the new setting takes effect.
"""

# ============================================================================
# THE BRAIN — which AI model powers Kareem
# ============================================================================
...
# done in kareem/agent.py) reduce wrong tool calls and misread requests, but
...
#    calls on Groq ("tool_use_failed" errors) which breaks Kareem's tools.
#    Free-tier limits (per model): ~30 requests/min plus daily token caps.
#    Plenty for personal use; if you hit them, Kareem's auto-retry waits it
...
#    canNOT call tools, so it would break Kareem's app/file/web abilities.
#
...
#            model metadata reports tools:false — Kareem's tool-calling on it
...
# with the local Ollama model instead (when it's running). Kareem prints a
...
# ============================================================================
# CLAUDE CODE DELEGATION (optional) — lets Kareem hand off coding tasks
# ============================================================================

# The directory Claude Code should treat as its project root when Kareem
# delegates a task to it. Defaults to Kareem's own project folder; change
# this if you want voice/text requests like "ask Claude to fix the bug in
# my other project" to target a different folder.
CLAUDE_CODE_PROJECT_DIR = None  # None = Kareem's own project root

# How Claude Code is allowed to act on this delegated task, since there's no
# interactive terminal for it to ask permission through (Kareem's own
...
#                 whatever Kareem might ask it to do completely.
...
# --- Google Calendar ---
# Lets Kareem create/list/delete events on your real Google Calendar when
...
# How often Kareem checks CMS/Portal/Mail for new deadlines while running,
...
# this setting).
# "backend" = your mic audio is sent to Kareem's own backend and transcribed
...
# If "backend" is selected but ffmpeg/faster-whisper aren't usable, Kareem
...
# Which text-to-speech engine speaks Kareem's replies.
...
#             kareem/voice/models/kokoro/ (see README).
# "pyttsx3" = the basic built-in Windows voice — always works, robotic.
# Whatever you pick, Kareem automatically falls back down kokoro -> piper ->
...
# Shows a small tray icon while Kareem is running, with a "Quit" option —
...
# Every action Kareem takes is written to this log file (created automatically).
LOG_FILE = "kareem.log"
