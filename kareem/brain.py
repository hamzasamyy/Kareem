...
a local Ollama model, and the Claude API without the rest of Kareem needing
to know which one is active.

Which brain is used is controlled entirely by kareem/config.py (BRAIN =
...
see kareem/tools/__init__.py) and an execute_tool(calls) callback, where
...
kareem.agent.Agent._execute_tool_calls — that's also where same-round calls
...
def strip_think(text: str) -> str:
    ...
    Kareem never prints (or reads aloud!) its internal thoughts. Also handles
...
class OllamaBrain:
    ...
    def chat(self, messages: list[dict], tools: list[dict] = None, execute_tool=None,
        ...
        on_tool_failure: optional callback(name, detail) fired for a
                     ...
                     See kareem.agent.Agent._notify_tool_failure.
        """
...
class HostedBrain:
    ...
    def __init__(self, model: str = None, base_url: str = None):
        ...
        # kareem/config.py (HOSTED_API_KEY_ENV), so switching providers —
        ...
        if not api_key:
            raise RuntimeError(
                ...
                f"Fix: add a line to .env (in the Kareem folder):\n"
        ...
        if not self.model or not self.base_url:
            raise RuntimeError(
                "HOSTED_BASE_URL / HOSTED_MODEL aren't set in kareem/config.py.\n"
        ...
        for attempt in range(3):
            try:
                ...
                if attempt < 2:
                    ...
                    # is frozen while it sleeps — a long wait here made Kareem
    ...
    def chat(self, messages: list[dict], tools: list[dict] = None, execute_tool=None,
        ...
        try:
            ...
            # conversation/connection — see kareem/web/server.py's
