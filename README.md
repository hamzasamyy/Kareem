...
> Automation, Windows toast notifications). Jarvis has only been built and
...
logged to `jarvis.log` in this folder.

### Optional Claude Code delegation

Jarvis can hand a coding task to the Claude Code CLI. This optional capability
...
commands, so Jarvis always asks for confirmation first, just like its other
risky actions. Its project folder and permission mode can be changed in
`jarvis/config.py`.

### Google Calendar (optional)

Jarvis can create, list, find, and delete events on your real Google Calendar.
...
Jarvis then stores `token.json` locally and refreshes access silently. Both
credential files are ignored by Git.

### GUC integration (optional, institution-specific)

Jarvis can automatically check CMS, the Student Portal, and Mail/OWA for new
...
short-lived, so Jarvis logs in fresh each check cycle rather than storing
one; credentials are read from `.env` only, never logged or written to
session transcripts. A background check runs every
`GUC_CHECK_INTERVAL_MINUTES` (`jarvis/config.py`, default 45) with
...
still something to use at your own judgment, not something Jarvis can
...
1. **The website (the main way)** — `python main.py` also starts a local web
   ...
   **Ctrl+Alt+J** opens/focuses it in your browser. It has streaming replies,
   a live feed of what Jarvis is doing, Confirm/Cancel buttons for risky
...
3. **Voice in the console** — set `WAKE_OPENS_WEB = False` in
   `jarvis/config.py` to make the wake word/hotkey talk out loud like it used
...
Kokoro is Jarvis's natural, offline speaking voice. Install its two Python
...
Create the folder `jarvis/voice/models/kokoro/` and put both files inside it.
To choose a different voice, change `TTS_VOICE` in `jarvis/config.py`; the
comments there list a few British and American choices. If Kokoro cannot
start, Jarvis automatically tries Piper and then the basic Windows voice.
...
```
python -m piper.download_voices en_US-lessac-medium --data-dir jarvis/voice/models
```
Restart Jarvis and set `TTS_ENGINE = "piper"` in `jarvis/config.py` if you
...
### Running Jarvis in the background (no console window)

Run this once from the Jarvis folder to create a **Jarvis** shortcut on your
Desktop:
...
Double-click that shortcut whenever you want to start Jarvis silently. A small
cyan tray icon shows that Jarvis is running; right-click it to **Open Web UI**
or **Quit**.

Because this keeps Jarvis running in the background, the **"hey jarvis"** wake
word now always has something listening — if it seemed not to work before,
that was because nothing was running.

To make Jarvis start automatically each time you sign in to Windows, run:
...
Open `jarvis/config.py` in Notepad — every setting is explained in comments:
...
What Jarvis treats as "risky" (= asks before doing) is listed in
`jarvis/safety.py` (`RISKY_ACTIONS`) and is meant to be edited by you.
...
latency, so replies start almost instantly and Jarvis begins **speaking the
...
**How the model id works:** in `jarvis/config.py`, `HOSTED_BASE_URL` picks
...
daily token caps — plenty for personal use. If you ever hit them, Jarvis
...
**Privacy note:** a few things Jarvis does leave your PC, depending on what
you use:
...
- **The vision-click fallback** (`jarvis/tools/vision.py`, last resort for
...
- **Web search** (`web_search`/`fetch_page`): your query, and any page URL
  you ask Jarvis to fetch, goes to DuckDuckGo / that page's own server.
...
**Your data:** everything Jarvis remembers is stored locally in this folder,
...
| Every action Jarvis takes (for debugging) | `jarvis.log` | Delete the file |
| Google Calendar authorization | `token.json` | Delete the file — Jarvis asks you to reconnect next time it's needed |

Jarvis doesn't automatically expire or delete any of this — it's yours to
...
Open `jarvis/config.py` and change one line:
```python
BRAIN = "ollama"
```
...
3. In `jarvis/config.py` set `BRAIN = "claude"`.
...
Jarvis/
  ...
  jarvis.log             # every action Jarvis takes is recorded here
  jarvis/
...
- **Wake word doesn't trigger** — speak clearly, close to the mic; check
  ...
  or lower `THRESHOLD` in `jarvis/voice/wakeword.py`.
